<script lang="ts">
	...
	export let livePreview = true;
	export let debounceMs = 200;

	// Output quality
	export let maxOutputPx = 2000; // clamp output width (higher = sharper, slower)
	...
	// Track whether knobs have been added as OSD overlays
	let knobsAdded = false;
	...
	function setKnobVisibility() {
		const visible = !!showGuides;
		if (topKnob) topKnob.style.display = visible ? 'block' : 'none';
		if (bottomKnob) bottomKnob.style.display = visible ? 'block' : 'none';
	}
	...
	function scheduleGeneratePreview() {
	...
	async function generatePreview() {
		...
		try {
			...
			// aspect from corner distances (in source px)
			const idxTL = 0;
			const idxTR = cols - 1;
			...
			// choose output width from panel width, clamp by maxOutputPx
			const panelW = hostEl?.clientWidth ?? 900;
			const dpr = window.devicePixelRatio || 1;
			const outW = Math.floor(clamp(panelW * dpr, 900, maxOutputPx));
	...
	// React to changes (debounced if livePreview)
	$: if (viewer && sourceUrl && mesh) {
		scheduleGeneratePreview();
	}

	// If you toggle guides / change count, just redraw (no regen)
	$: if (viewer && svgEl) {
		scheduleGuidesRedraw();
	}
...
<div class="shell">
	...
	<div class="toolbar">
		...
		<label><input type="checkbox" bind:checked={livePreview} /> Live</label>

		<button
			class="btn"
			on:click={() => generatePreview()}
			disabled={!viewer || !sourceUrl || !mesh || generating}
		>
			Update
		</button>
	</div>
